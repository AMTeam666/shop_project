                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($sms as $key => $singleSms)
                        <tr>
                            ...
                            <td>{{ Str::limit($singleSms->body, 20)  }}</td>
                            <td>{{ jalaliDate($singleSms->published_at, 'H:i:s Y-m-d') }}</td>
                            <td>
                                <label>
                                    <input id="{{ $singleSms->id }}" onchange="changeStatus({{ $singleSms->id }})" data-url="{{ route('admin.notify.sms.status', $singleSms->id) }}" type="checkbox" @if ($singleSms->status === 1)
                                        checked
                                        @endif>
                                </label>
                            </td>
