    ...
    <section class="col-12">
        <section class="main-body-container">
            ...
            <section class="d-flex justify-content-between align-items-center mt-4 mb-3 border-bottom pb-2">
                <a href="{{ route('admin.user.sale.create') }}" class="btn btn-info btn-sm">ایجاد مشتری جدید</a>
                ...
                <table class="table table-striped table-hover">
                    ...
                    @foreach($salesMans as $key => $user)
                        <tr>
                            ...
                            <td>
                                <label>
                                    <input id="{{ $user->id }}-active" onchange="changeActivation({{ $user->id }})" data-url="{{ route('admin.user.sale.activation', $user->id) }}" type="checkbox" @if ($user->activation === 1)
                                        checked
                                        @endif>
                                </label>
                            </td>
                            ...
                            <td class="width-22-rem text-left">
                                <a href="{{ route('admin.user.sale.edit', $user->id) }}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> ویرایش</a>
                                <form class="d-inline" action="{{ route('admin.user.sale.destroy', $user->id) }}" method="post">
                                    @csrf
                                    {{ method_field('delete') }}
                                    <form class="d-inline" action="{{ route('admin.user.sale.destroy', $user->id) }}" method="post">
                                        @csrf
